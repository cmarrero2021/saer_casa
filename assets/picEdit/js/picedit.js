    ...
    Plugin.prototype = {
		...
		set_loading: function (message) {
			if(message && message == 1) {
				return this.set_messagebox("Trabajando...", false, false);
			}
			else return this.set_messagebox("Por favor espere...", false, false);
		},
		...
		camera_open: function() {
			...
			var browserUserMedia = navigator.webkitGetUserMedia	||	// WebKit
									 ...
									 if (!browserUserMedia) return this.set_messagebox("Su navegador no tiene soporte para WebRTC!");
									 var _this = this;
				...
				function(err) {
					return _this.set_messagebox("No se ha detectado una fuente de video; por favor autorice el acceso a la cámara de su navegador");
				}
				);
									},
		...
		_hideAllNav: function (message) {
			if(message && message == 1) {
				this.set_messagebox("Abra una imágen o utilice su cámara para tomar una foto");
			}
		...
		_formsubmit: function() {
			if(!window.FormData) this.set_messagebox("Lamentablemente la API FormData ya no tiene soporte");
			else {
				var _this = this;
				this.set_loading().delay(200).promise().done(function() {
					...
					request.onprogress = function(e) {
						...
						_this.set_messagebox("Por favor espere... Cargando... " + progress + "% Cargado.", false, false);
					};
					request.open(_this._theform.prop("method"), _this._theform.prop("action"), true);
					request.onload = function(e) {
						if(this.status != 200) {
							_this.set_messagebox("El Servidor no aceptó la imagen");
							alert("El Servidor no aceptó la imagen");
						}
						else {
							...
							else _this.set_messagebox("La imagen fué subida exitosamente");
							alert("La imagen fué subida exitosamente");
						}
