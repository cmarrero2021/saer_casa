	...
	function fecha_actual() {
		var x = new Date();
		var titulo = "tipo_"+x.getFullYear()+""+("0"+(x.getMonth() +1)).slice(-2)+""+("0"+x.getDate()).slice(-2)+"_"+("0"+x.getHours()).slice(-2)+""+("0"+x.getMinutes()).slice(-2)+""+("0"+x.getSeconds()).slice(-2);
		return titulo;		
	}
	...
	$(document).ready(function() {
		$('#item-list').DataTable({
			...
			buttons: [
			{
				text: "<i class='icon-plus btn btn-lg btn-bluegray'></i>",titleAttr: "Agregar un tipo nuevo",
				action: function (e, dt, node, config) {
					accion_guardar = 'add';
					$n=0;
					$("#tipo option").each(function(){
						if ($(this).text() == "Seleccione un tipo de tipo") {
							$n++;
						}
					});
					if ($n == 0) {
						$('#tipo').prepend('<option value="0" selected>Seleccione un tipo de tipo</option>');
					}
					$m=0;
					...
					$('#tipo option').show();
					$('#estado option').show();
					$('.modal-title').text('Agreagr un nuevo tipo');
				}
			},
	...
	function guardar1(){
		if (accion_guardar == 'add') {
			url=base_url+'tipos/ajax_agregar';
			$("#id_tipo").val(0);
		} else {
			url=base_url+'tipos/ajax_actualizar';
		}
		var datos = $('#tipo_form').serialize();
		console.log(datos);
		$.ajax({
			url: url,
			type: 'POST',
	...
	function eliminar1(id) {
		swal({
			...
			if (result.value) {
				var url;
				url = base_url+'tipos/ajax_eliminar/'+id;
				$.ajax({
					url: url,
					type: 'GET',
					dataType: 'JSON',
	...
	function editar1(id) {
		...
		$("#id_registro").show();
		$.ajax({
			url: base_url+"tipos/ajax_editar/" + id,
			type: 'GET',
			dataType: 'JSON',
			data:id,
		})
		.done(function(data) {
			$('#id_tipo').val(data.id_tipo_aeropuerto);
			$('#etq_id_tipo').html(data.id_tipo_aeropuerto);
			$('[name="tipo"]').val(data.tipo_aeropuerto);
			$('#userModal').modal('show');
			$('.modal-title').text('Editar tipo de aeropuerto');
		})
		.fail(function() {
			swal(
				'¡ERROR EN LA OPERACIÓN!',
	...
	function ficha1(id) {
		...
		.done(function(data) {
			$('#id_tipo_ficha').text(data.id_tipo_aeropuerto);
			$('#tipo_ficha').text(data.tipo_aeropuerto);
			$('#vistaModal').modal('show');
			$('.modal-title').text('Ficha del tipo de aeropuerto');
		})
		.fail(function() {
			swal(
				'¡ERROR EN LA OPERACIÓN!',
