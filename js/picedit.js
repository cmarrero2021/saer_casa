    ...
    Plugin.prototype = {
		...
		camera_open: function() {
			...
			var browserUserMedia = navigator.webkitGetUserMedia	||	// WebKit
									 ...
									 if (!browserUserMedia) return this.set_messagebox("Lamentablemente su navegador no tiene soporte para WebRTC");
									 var _this = this;
				...
				function(err) {
					return _this.set_messagebox("No se ha detectado ninguna fuente de video; por favor autorice el acceso a la cámara por parte del navegador");
				}
				);
									},
		...
		_hideAllNav: function (message) {
			if(message && message == 1) {
				this.set_messagebox("Abra una imagen o utilice una cámara para tomar una foto");
			}
		...
		_formsubmit: function() {
			if(!window.FormData) this.set_messagebox("Lamentablemente no hay soporte para la API FormData");
			else {
				var _this = this;
				this.set_loading().delay(200).promise().done(function() {
